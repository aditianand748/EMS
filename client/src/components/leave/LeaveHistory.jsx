                      ...
                      leaves.map((leave)=>{
                        return (
                          <tr key={leave._id || leave.id}>
                           ...
                           <td>
                            <span className={`badge ${leave.status === "APROVED" ?
                           ...
                           {isAdmin && (
                            ...
                            {leave.status === "PENDING" && (
                                ...
                                <button
                                 disabled = {!!processing}
                                onClick={()=> handleStatusUpdate(leave._id || leave.id, "APROVED")} className='p-1.5 rounded-md 
                                bg-emerald-50 text-emerald-600 
                                hover:bg-emerald-100 transition-colors'>
                                    {processing === (leave._id || 
                                        leave.id) ? <Loader2Icon className='w-4 h-4 animate-spin'/> : <Check className='w-4 h-4'/>}
                                </button>
                                 ...
                                 hover:bg-rose-100 transition-colors'>
                                    {processing === (leave._id || 
                                        leave.id) ? <Loader2Icon className='w-4 h-4 animate-spin'/> : <X className='w-4 h-4'/>}
                                </button>
                                </div>
                            )} 
                           </td>
